...
// Read parent container ID from global (set by React page), default 'game'
var GAME_PARENT = (window.__GAME_CONFIG && window.__GAME_CONFIG.parent) || 'game';
...
// ========== MULTIPLAYER GLOBALS ==========
var remotePlayerSprite = null;
var remoteTarget = { x: 0, y: 0 };
var remoteConnected = false;
var __touchControls = { left: false, right: false, jump: false, fire: false };
var LERP_FACTOR = 0.2;
var TELEPORT_THRESHOLD = 150;
...
function preload() {
    ...
    // Set base URL so original relative asset paths work from any page
    this.load.baseURL = '/assets/FullGame/';
...
function create() {
    ...
    // Multiplayer init (requires __multiplayer global set by React page)
    if (typeof initMultiplayer === 'function') initMultiplayer.call(this);
}

function createControls() {

    // Touch input bridge — React TouchControls updates __touchControls global, joystick reads it
    this.joyStick = {
        get left() { return __touchControls.left; },
        get right() { return __touchControls.right; },
        get up() { return __touchControls.jump; },
        get down() { return __touchControls.fire; }
    };
...
function update(delta) {
    ...
    // Multiplayer sync
    if (typeof syncToFirebase === 'function') syncToFirebase.call(this);
    if (typeof updateRemotePlayer === 'function') updateRemotePlayer.call(this);
...
function initMultiplayer() {
    if (!window.__multiplayer) return;

    // Create remote player sprite (blue ghost)
    ...
    g.generateTexture('_remotePlayer', 32, 48);
    g.clear();
    g.destroy();

    remotePlayerSprite = this.physics.add.sprite(-100, -100, '_remotePlayer');
    remotePlayerSprite.setVisible(true);
    remotePlayerSprite.setDepth(5);
    var rb = remotePlayerSprite.body;
    rb.allowGravity = false;
    rb.moves = false;
    rb.immovable = true;
    rb.checkCollision.none = true;
...
function syncToFirebase() {
    if (!window.__multiplayer || !player || !player.body) return;
    window.__localState = {
...
function updateRemotePlayer() {
    if (!remotePlayerSprite || !remoteConnected) return;
    var dist = Phaser.Math.Distance.Between(
        remotePlayerSprite.x, remotePlayerSprite.y,
        remoteTarget.x, remoteTarget.y
    );
    if (dist > TELEPORT_THRESHOLD) {
        remotePlayerSprite.x = remoteTarget.x;
        remotePlayerSprite.y = remoteTarget.y;
    } else {
        remotePlayerSprite.x = Phaser.Math.Linear(remotePlayerSprite.x, remoteTarget.x, LERP_FACTOR);
        remotePlayerSprite.y = Phaser.Math.Linear(remotePlayerSprite.y, remoteTarget.y, LERP_FACTOR);
    }
}

// Called by React page to update remote player position from Firebase
function setRemotePosition(x, y, facing) {
    remoteTarget.x = x;
    remoteTarget.y = y;
    remoteConnected = true;
    if (remotePlayerSprite) {
        remotePlayerSprite.setVisible(true);
        if (facing !== undefined) remotePlayerSprite.setFlipX(facing < 0);
    }
}

// Called by React page when remote player disconnects
function clearRemotePlayer() {
    remoteConnected = false;
    if (remotePlayerSprite) {
        remotePlayerSprite.setPosition(-100, -100);
    }
}

// Expose multiplayer API on window for React bridge
window.__multiplayerAPI = {
    setRemotePosition: setRemotePosition,
    clearRemotePlayer: clearRemotePlayer,
    getLocalState: function() { return window.__localState || null; }
};

// ========== SETTINGS STUBS (Rex plugins not available) ==========
function showSettings() { /* no-op */ }
function hideSettings() { /* no-op */ }
function drawSettingsMenu() { /* no-op */ }
function applySettings() { /* no-op */ }
